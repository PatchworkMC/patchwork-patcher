...
public class PatchworkApplication {
    private static class Commandline {
        ...
        @Parameter(name = "another file", description = "The other file\nIts a cooler one!", position = 1)
        String fileTwo;

        @Parameter(name = "optional file", description = "Nobody needs this file, you can still supply it", position = 2, required = false)
        String optionalFile;
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            logger.setWriter(new StreamWriter(!commandline.noColors, System.out, System.err), LogLevel.TRACE);
        ...
        if(!parser.parseSucceeded() || commandline.help) {
            System.out.println(parser.generateHelpMessage(
                    getExecutableName(),
    ...
    private static String getExecutableName() {
        try {
            URL location = PatchworkApplication.class.getProtectionDomain().getCodeSource().getLocation();
