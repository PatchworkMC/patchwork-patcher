...
public class SubscribeEventGenerator {
	public static String generate(String targetClass, SubscribeEvent entry, ClassVisitor visitor) {
		...
		ConsumerGenerator generator =
				new ConsumerGenerator(visitor, shimName, eventDescriptor, signature);

		// Add a default constructor

		if(instance) {
			System.err.println("Instance subscribe event on " + targetClass +
							   " (method: " + entry.getMethod() + ")");

			// TODO: Generic parents? They should have a signature here

			generator.getVisitor().visitField(Opcodes.ACC_PRIVATE | Opcodes.ACC_FINAL,
					"instance",
					targetDescriptor,
					null,
					null);
			...
			MethodVisitor constructor =
					generator.visitConstructor("(" + targetDescriptor + ")V", null);
		...
		method.visitMethodInsn(instance ? Opcodes.INVOKEVIRTUAL : Opcodes.INVOKESTATIC,
