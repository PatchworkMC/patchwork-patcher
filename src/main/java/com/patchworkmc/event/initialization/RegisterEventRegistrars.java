	...
	public void accept(MethodVisitor method) {
		...
		for (EventSubscriber subscriber : eventSubscribers) {
			...
			if (subscriber.hasInstanceSubscriber()) {
				...
				LambdaUtil.visitBiConsumerStaticMethodReference(method, className, EventConstants.REGISTER_STATIC, EventConstants.REGISTER_STATIC_DESC, subscriber.isInterface());
