...
import com.patchworkmc.generator.BiConsumerGenerator;

public class InstanceEventRegistrarGenerator {
	...
	public static String generate(String targetClass, Collection<Map.Entry<String, SubscribeEvent>> entries, ClassVisitor visitor) {
		...
		for (Map.Entry<String, SubscribeEvent> entry : entries) {
			...
			subscriber.getGenericClass().ifPresent(genericClass -> method.visitLdcInsn(Type.getObjectType(genericClass)));

			// Adds the event priority
			method.visitFieldInsn(Opcodes.GETSTATIC, "net/minecraftforge/eventbus/api/EventPriority", subscriber.getPriority(), "Lnet/minecraftforge/eventbus/api/EventPriority;");
			...
			// Load the instance on to the stack
			method.visitVarInsn(Opcodes.ALOAD, 1);

			method.visitMethodInsn(Opcodes.INVOKESPECIAL, shimName, "<init>", "(" + targetDescriptor + ")V", false);
