...
public class AccessTransformerConverter {
	...
	public static byte[] convertToWidener(ForgeAccessTransformer accessTransformer, TargetMappingsHolder holder) {
		...
		for (TransformedClass targetClass : accessTransformer.getClasses()) {
			...
			try {
				writeWildcards(sb, targetClass.getName(), targetClass.getFieldWildcard(), targetClass.getMethodWildcard(), holder);
	...
	private static void writeWildcards(StringBuilder sb, String owner, TransformedWildcardMember fields, TransformedWildcardMember methods,
				TargetMappingsHolder holder) throws MissingMappingException {
		if (fields != null) {
			holder.getMappings(owner).fields.forEach((key, field) -> writeField(sb, owner, field.name, field.descriptor, fields));
		}

		if (methods != null) {
			holder.getMappings(owner).methods.forEach((key, method) -> writeMethod(sb, owner, method.name, method.descriptor, methods));
		}
	}

	private static void writeField(StringBuilder sb, TransformedField transformed, TargetMappingsHolder holder) throws MissingMappingException {
		String owner = transformed.getOwner();
		String name = transformed.getName();
		TargetMappingsHolder.Member targetMember = holder.getMappings(owner).fields.get(name);
